...
Page({
  ...
  selectCoupon () {
    ...
    wx.navigateTo({
      url:
        '/pGoods/pages/goodsSelectCoupon/goodsSelectCoupon?price=' +
        this.data.info.price * this.data.count +
        '&goodsId=' +
        this.data.info.id +
        '&id=' +
      ...
      events: {
        useCoupon ({ data }) {
          ctx.setData({
            curCoupon: data
              ? ctx.data.coupons.filter(e => e.id === data)[0] || null
              : null
          })
          ctx.calcDiscount()
        }
  ...
  loadData () {
    ...
    Promise.all([
      ...
      apiService('product/detail', 'GET', {
        ...
        .catch(() => {}),
        apiService('common/get/scoreUsage', 'GET').then(rst => {
          percent = Math.floor(rst.usablePercent)
        })
    ]).then(() => {
      wx.hideLoading()
      let price = info.price * (this.data.count || 1)
  ...
  calcDiscount () {
    if (!this.data.curCoupon) {
      ...
      this.data.discount =
        this.data.curCoupon.couponType === 1
          ? this.data.curCoupon.couponValue
          : Math.round(
              this.data.info.price *
                this.data.count *
                (1 - this.data.curCoupon.couponValue / 100) *
                100
            ) / 100
    }
    const total = Math.round(
      (this.data.info.price * this.data.count - this.data.discount) * 100
    ) / 100
  ...
  confirmOrder () {
    ...
    if (
      !this.data.info.unlimitStock &&
      this.data.info.stock < this.data.count
    ) {
      wx.showToast({
        title: '库存不足',
        icon: 'none',
    ...
    apiService('order/create', 'POST', {
          ...
          wx.requestPayment({
            ...
            success: function (res) {
              wx.showToast({
                ...
                complete () {
                  wx.redirectTo({
                    url: '/pGoods/pages/goodsOrderList/goodsOrderList'
                  })
                }
              })
      ...
      .catch(err => {
        ...
        if (err.data) {
          wx.showToast({
            title: err.data && err.data.errorMessage,
            icon: 'none',
            duration: 2000
          })
        }
