...
import { marketing } from './api/marketingApi.js'
import { config } from './config/config.js'
let marketApi = new marketing()
import { svip } from './api/svipApi.js'
...
const util = {
      ...
      marketApi
        ...
        .then((res) => {
          ...
          if (res.status == 1) {
            ...
            Taro.setStorageSync('userInfo', resData.user_info)
            ...
            Taro.request({
              ...
              success(resp) {
                ...
                Taro.setStorageSync('mall_token', resp.data.data)
              },
              fail(err) {
                console.log('失败了')
              },
            ...
            Taro.uma.trackEvent('Event_LoginSuc', {
  ...
  getPositionCity(src = '', callBack) {
    ...
    Taro.getLocation({
      ...
      complete() {
        Taro.setStorageSync('isLocation', true)
      },
    })
    // },
    // })
  ...
  loadCity: function (longitude, latitude, resData, callBack) {
    ...
    Taro.request({
      url:
        'https://api.51jiabo.com/msv/rest/v1/map/getcityinfobyxy.do?x=' +
        longitude +
        '&y=' +
        latitude,
      ...
      success: function (res) {
        ...
        resData.forEach((item) => {
          ...
          if (item.id == login_id) {
            Taro.setStorageSync('curUserCityText', item.city_name)
            Taro.setStorageSync('cityId', item.id) // 老项 新项 通用
            Taro.hideLoading()
            Taro.setStorage({
              key: 'cityId',
              data: item.id,
  ...
  getGoodsPrice(goodsInfo) {
    ...
    if (goodsInfo.catalog === 0) {
      ...
      return Taro.getStorageSync('isSvip')
        ? goodsInfo.catalogValue
