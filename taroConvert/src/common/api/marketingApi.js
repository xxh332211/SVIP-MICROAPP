...
class marketing extends httpAsync {
  ...
  cloudShowInfo(data) {
    return this.requestNew({
      url:
        '/v2.0/city/cloudShow?cityId=' + data.cityId + '&mobile=' + data.mobile,
    })
  }
  ...
  getVendorAction(data) {
    return this.requestNew({
      url:
        '/expo/supplier_active/list?category=' +
        data.id +
        '&page=' +
        data.page +
        '&pageSize=' +
  ...
  getBrandList(data) {
    return this.requestNew({
      url:
        '/expo/brand/list?category=' + data.id + '&pageSize=' + data.pageSize,
    })
  }
